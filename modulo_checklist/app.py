...
def index():
    ...
    if connection:
        try:
            ...
            # Total de modelos
            cursor.execute("SELECT COUNT(*) as total FROM checklist_modelos WHERE ativo = TRUE")
            resultado = cursor.fetchone()
            estatisticas['total_modelos'] = resultado['total'] if resultado else 0
            
            # Checklists pendentes
            cursor.execute("SELECT COUNT(*) as total FROM checklist_preenchidos WHERE status = 'em_andamento'")
            resultado = cursor.fetchone()
            estatisticas['checklists_pendentes'] = resultado['total'] if resultado else 0
            
            # Checklists concluídos
            cursor.execute("SELECT COUNT(*) as total FROM checklist_preenchidos WHERE status IN ('concluido', 'aprovado')")
            resultado = cursor.fetchone()
            estatisticas['checklists_concluidos'] = resultado['total'] if resultado else 0
            
            # Checklists de hoje
            cursor.execute("SELECT COUNT(*) as total FROM checklist_preenchidos WHERE DATE(data_preenchimento) = CURDATE()")
            resultado = cursor.fetchone()
            ...
            if filtro_equipamento:
                ...
                params.extend([f"%{filtro_equipamento}%", f"%{filtro_equipamento}%"])
    ...
    if connection:
        try:
            ...
            if checklist['status'] not in ['em_andamento']:
                flash('Este checklist já foi concluído e não pode ser editado', 'warning')
            ...
            if request.method == 'POST' and request.form.get('acao') == 'salvar':
                ...
                for item in itens:
                    ...
                    resposta_numerica = request.form.get(prefix + 'numerica', None)
                    resposta_booleana = request.form.get(prefix + 'booleana') == 'sim'
                    conformidade = request.form.get(prefix + 'conformidade', 'conforme')
                    ...
                    if resposta_numerica and resposta_numerica.strip():
                        try:
                            resposta_numerica = float(resposta_numerica.replace(',', '.'))
                        except:
                ...
                if concluir:
                    ...
                    """, [checklist['modelo_id']])
                    total_obrigatorios = cursor.fetchone()['total_obrigatorios']
                    ...
                    if total_respondidos < total_obrigatorios:
                        flash('Responda todos os itens obrigatórios antes de concluir o checklist', 'warning')
                    else:
...
def aprovar_checklist(checklist_id):
    ...
    if connection:
        try:
            ...
            # Buscar o checklist
            cursor.execute("SELECT * FROM checklist_preenchidos WHERE id = %s", [checklist_id])
            checklist = cursor.fetchone()
    ...
    def inject_menu_data():
        ...
        if 'logado' in session:
            menu_items = [
                ...
                {'name': 'Solicitações', 'url': url_for('dashboard'), 'icon': 'fas fa-clipboard-list'},
                {'name': 'Notas Fiscais', 'url': url_for('importacao_nf.index'), 'icon': 'fas fa-file-invoice'},
                {'name': 'Checklists', 'url': url_for('checklist.index'), 'icon': 'fas fa-tasks'},
                {'name': 'Integração ERP', 'url': url_for('integracao_erp.index'), 'icon': 'fas fa-sync'},
                {'name': 'Relatórios', 'url': url_for('relatorios'), 'icon': 'fas fa-chart-bar'},
            ]
        return {'menu_items': menu_items}2 ON c.supervisor_id = u2.id
...
def listar_modelos():
    ...
    if connection:
        try:
            ...
            cursor.execute("SELECT DISTINCT tipo_equipamento FROM checklist_modelos WHERE ativo = TRUE ORDER BY tipo_equipamento")
            tipos_equipamento = [row['tipo_equipamento'] for row in cursor.fetchall()]
...
def editar_modelo(modelo_id):
    ...
    if connection:
        try:
            ...
            # Buscar o modelo
            cursor.execute("SELECT * FROM checklist_modelos WHERE id = %s", [modelo_id])
...
def adicionar_item(modelo_id):
    ...
    try:
        ...
        valores_possiveis = json.dumps(data.get('valores_possiveis', [])) if data.get('valores_possiveis') else None
        ...
        try:
            ...
            cursor.execute("SELECT * FROM checklist_itens WHERE id = %s", [item_id])
...
def gerenciar_item(item_id):
    ...
    try:
        ...
        if request.method == 'DELETE':
            ...
            cursor.execute("UPDATE checklist_itens SET ativo = FALSE WHERE id = %s", [item_id])
            connection.commit()
    ...
    try:
        ...
        try:
            ...
            for ordem, item_id in enumerate(item_ids, 1):
                cursor.execute("UPDATE checklist_itens SET ordem = %s WHERE id = %s", [ordem, item_id])
...
def listar_checklists():
    ...
    if connection:
        try:
            ...
            cursor.execute("SELECT id, nome FROM checklist_modelos WHERE ativo = TRUE ORDER BY nome")
            modelos = cursor.fetchall()
