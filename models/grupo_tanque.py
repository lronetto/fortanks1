...
class GrupoTanque(db.Model):
    """
    Modelo para representar grupos de tanques
    """
    __tablename__ = 'grupos_tanques'
    ...
    # Relacionamentos
    tanques = relationship('Tanque', secondary='tanques_grupos', back_populates='grupos')
    
    def __repr__(self):
        return f'<GrupoTanque {self.id} - {self.nome}>'
...
tanques_grupos = db.Table('tanques_grupos',
    db.Column('tanque_id', db.Integer, db.ForeignKey('tanques.id'), primary_key=True),
    db.Column('grupo_id', db.Integer, db.ForeignKey('grupos_tanques.id'), primary_key=True),
