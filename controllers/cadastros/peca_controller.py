...
def index():
    ...
    # Filtros de status (positivos)
    filtro_concretado = request.args.get('filtro_concretado', type=str) == '1'
    filtro_produzido = request.args.get('filtro_produzido', type=str) == '1'
    ...
    # Filtros de status (negativos)
    filtro_nao_concretado = request.args.get('filtro_nao_concretado', type=str) == '1'
    ...
    filtros_iniciais_dict = {
        ...
        'filtro_nao_concretado': filtro_nao_concretado,
    ...
    return render_template('cadastros/pecas/index.html',
    ...
    try:
        ...
        for peca in pecas_todas:
            ...
            if filtro_nao_concretado and concretado:
                continue
            if filtro_nao_produzido and produzido:
                continue
            if filtro_nao_acabado and acabada:
                continue
        ...
        for peca in pecas_filtradas:
            ...
            if produzido:
                status_badges.append('<span class="badge bg-warning me-1"><i class="fas fa-industry"></i> Produzido</span>')
            if acabada:
            ...
            tanque_nome_esc = (peca.tanque.nome if peca.tanque else 'N/A').replace('"', '&quot;')
            acoes_html = (
                ...
                f'<li><a class="dropdown-item" href="{url_visualizar}"><i class="fas fa-eye text-info"></i> Visualizar</a></li>'
                f'<li><a class="dropdown-item" href="{url_editar}"><i class="fas fa-edit text-warning"></i> Editar</a></li>'
                f'<li><hr class="dropdown-divider"></li>'
                f'<li><button type="button" class="dropdown-item text-danger btn-excluir-peca"'
                f' data-peca-id="{peca.id}" data-peca-nome="{peca.nome.replace(chr(34), "&quot;")}"'
                f' data-tanque-nome="{tanque_nome_esc}" data-numero-seq="{peca.numero_sequencial}">'
...
@peca.route('/exportar/excel', methods=['GET'])
@login_required
def exportar_excel():
    """Exporta peças filtradas para Excel (respeitando os mesmos filtros da listagem)."""
    try:
        tanque_ids = request.args.getlist('tanque_ids[]', type=int)
        if not tanque_ids:
            tanque_id = request.args.get('tanque_id', type=int)
        ...
        for peca in pecas_filtradas:
            ...
            rows.append({
                ...
                'Projeto': peca.tanque.contrato.nome if peca.tanque and peca.tanque.contrato else 'Sem projeto',
                'Tipo': peca.tipo or '',
                'Nome': peca.nome or '',
                'Concretado': 'Sim' if concretado else 'Não',
        ...
        output.seek(0)
        filename = f'pecas_{datetime.now().date().isoformat()}.xlsx'
        return send_file(
            output,
...
def listar_por_tanque(tanque_id):
    ...
    return render_template('cadastros/pecas/listar.html', pecas=pecas, tanque=tanque)
...
def novo(tanque_id):
    ...
    return render_template('cadastros/pecas/novo.html', tanque=tanque, proximo_sequencial=proximo_sequencial)
...
def editar(id):
    ...
    return render_template('cadastros/pecas/editar.html', peca=peca)

@peca.route('/visualizar/<int:id>')
def visualizar(id):
    """Visualiza detalhes de uma peça"""
    peca = TanquesPecas.query.get_or_404(id)
    return render_template('cadastros/pecas/visualizar.html', peca=peca)
...
def acabamento():
    ...
    return render_template('cadastros/pecas/modais/acabamento.html', tanques=tanques, csrf_token=csrf_token)
...
def transporte():
    ...
    tanques = Tanques.query.order_by(Tanques.nome).all()
    csrf_token = generate_csrf()
    return render_template('cadastros/pecas/modais/transporte.html', tanques=tanques, csrf_token=csrf_token)
...
def vinculacao_produto_composto():
    ...
    return render_template('cadastros/pecas/modais/vinculacao_produto_composto.html', 
                         contratos=contratos, 
...
def processar_arquivo_inspecao(xlsx_path):
    ...
    for index, row in df_series.iterrows():
        ...
        #print(f"Procurando produto composto: {get_value_str(row, 5)}")
        produto_composto_id = ProdutoComposto.query.filter(
            ProdutoComposto.nome.like(f'%{get_value_str(row, 5)}%'),
            ProdutoComposto.traco==True).first().id
        serie['produto_composto_id'] = produto_composto_id
