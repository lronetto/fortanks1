...
def index():
    ...
    return render_template('cadastros/usuarios/index.html', cargos=cargos, departamentos=departamentos)
...
def api_dados():
    ...
    for usuario in usuarios:
        ...
        botoes_acao = (
            ...
            f'<li><button type="button" class="dropdown-item btn-editar-usuario"'
            f' data-id="{usuario.id}" data-nome="{usuario.nome}" data-email="{usuario.email}"'
            f' data-departamento-id="{usuario.departamento_id}" data-cargo-id="{usuario.cargo_id}">'
            f'<i class="fas fa-edit text-primary"></i> Editar</button></li>'
            f'<li><button type="button" class="dropdown-item btn-resetar-senha"'
            f' data-id="{usuario.id}" data-nome="{usuario.nome}">'
            f'<i class="fas fa-key text-warning"></i> Resetar Senha</button></li>'
        )
        ...
        if usuario.id != current_user.id:
            botoes_acao += (
                ...
                f'<li>'
                f'<form id="form-excluir-{usuario.id}" action="{url_for("usuario.excluir", id=usuario.id)}" method="POST" class="d-inline form-excluir">'
...
def novo():
    ...
    if request.method == 'POST':
        ...
        if not nome or not email or not senha or not departamento_id or not cargo_id:
            ...
            cargos = Cargo.query.filter_by(status='Ativo').order_by(Cargo.nome).all()
            departamentos = Departamento.query.filter_by(status='Ativo').order_by(Departamento.nome).all()
            return render_template('cadastros/usuarios/novo.html', cargos=cargos, departamentos=departamentos)
        ...
        novo_usuario = Usuario(
            nome=nome,
            email=email,
            senha=Usuario.hash_password(senha),
    ...
    return render_template('cadastros/usuarios/novo.html')
    ...
    if request.method == 'POST':
        ...
        if not nome or not email or not departamento_id or not cargo_id:
            if is_ajax:
                return jsonify({'success': False, 'message': 'Por favor, preencha todos os campos.'}), 400
            flash('Por favor, preencha todos os campos.', 'danger')
            return render_template('cadastros/usuarios/editar.html', usuario=usuario)
    ...
    if request.method == 'POST':
        ...
        if not nova_senha or not confirmar_senha:
            if is_ajax:
                return jsonify({'success': False, 'message': 'Por favor, preencha todos os campos.'}), 400
            flash('Por favor, preencha todos os campos.', 'danger')
            return render_template('cadastros/usuarios/resetar_senha.html', usuario=usuario)
