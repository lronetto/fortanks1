"""Endpoints JSON/AJAX do domínio Material."""
...
def api_buscar_semelhantes():
    try:
        ...
        if codigo:
            termo = f"%{codigo}%"
            mats = Materiais.query.filter(
                Materiais.dados_adicionais.ilike(termo), Materiais.ativo.is_(True)
            ).limit(5).all()
            for mat in mats:
                materiais_encontrados.append(
                    {"material": mat, "pontuacao": 70, "motivo": "Código (dados adicionais)"}
                )
                ...
                if len(palavra) >= 3:
                    materiais_nome = (
                        Materiais.query.filter(
                            db.or_(
                                Materiais.nome.ilike(f"%{palavra}%"),
                                Materiais.descricao.ilike(f"%{palavra}%"),
                            ),
                    ...
                    for mat in materiais_nome:
                        ...
                        if existente:
                            ...
                            palavras_match = sum(
                                1 for p in palavras if p.lower() in nome_lower or p.lower() in desc_lower
                            )
                            pontuacao = 30 + (palavras_match * 15)
                            materiais_encontrados.append(
                                {"material": mat, "pontuacao": pontuacao, "motivo": "Nome/Descrição similar"}
                            )
            ...
            if remover_img:
                ...
                if new_uid:
                    if old_uid and old_uid != new_uid:
                        remover_upload_material_se_existir(old_uid, material.id)
                    material.dados_adicionais = salvar_imagem_material(material.id, arquivo_img)
