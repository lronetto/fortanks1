...
def build_matriz_semanal_xlsx_bytes(
    *,
    ...
    ws1.set_row(0, 38)
    ws1.merge_range(0, 0, 0, 2 + n, f'Matriz semanal — {nome_projeto}', fmt_title)
    ...
    for j in range(n):
        ...
        ws1.write(header_row, 3 + j, f'{lab}\n{lf}', fh)
    ...
    for i, row in enumerate(linhas):
        ...
        for j in range(n):
            ...
            ws1.write(r, 3 + j, txt, fc)

    ws1.set_column(0, 0, 10)
    ws1.set_column(1, 1, 36)
    ws1.set_column(2, 2, 11)
    for j in range(n):
        ws1.set_column(3 + j, 3 + j, 11)
    if n > 0:
        ws1.freeze_panes(header_row + 1, 3)

    # --- Aba 2: Dias (heatmap no tempo) ---
    ws2 = workbook.add_worksheet('Dias previsto')
    ws2.merge_range(0, 0, 0, 2 + n, f'Dias de trabalho previstos — {nome_projeto}', fmt_title)
    ...
    if barras_dias_semana and len(barras_dias_semana) == n:
        ...
        for j in range(n):
            ...
            ws2.write_number(tr, 3 + j, val, f)

    ws2.set_column(0, 0, 10)
    ws2.set_column(1, 1, 36)
    ws2.set_column(2, 2, 11)
    for j in range(n):
        ws2.set_column(3 + j, 3 + j, 10)
    if n > 0:
        ws2.freeze_panes(hrow + 1, 3)
