                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    for att in msg.attachments:
                        if att.filename.lower().endswith('.xml'):
                            ...
                            try:
                                print(f"Processando nota fiscal: ")
                                tinicial=time.time()
                                nnf = processar_nota_fiscal_xml(xml_text)
                                tfinal=time.time()
                            ...
                            cc = get_CC(nnf)
                            print(f"CC: {cc} NNF: {nnf}")
                            upload = upload_arquivei(base64.b64encode(xml_text.encode('utf-8')).decode('utf-8'))
